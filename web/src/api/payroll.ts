...
export async function downloadPayslipPdf(payrollId: number, name: string): Promise<void> {
  const res = await api.get(`/payrolls/${payrollId}/payslip.pdf`, { responseType: "blob" });
  const url = window.URL.createObjectURL(res.data);
  const a = document.createElement("a");
  a.href = url;
  a.download = `payslip_${name}.pdf`;
