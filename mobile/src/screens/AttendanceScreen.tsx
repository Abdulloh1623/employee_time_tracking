...
function statusColors(c: Colors): Record<string, string> {
  return {
    present: c.green, late: c.orange, absent: c.red, on_leave: c.brand, sick: c.grape, day_off: c.dim,
  };
}
...
export default function AttendanceScreen() {
  ...
  const load = useCallback(async () => {
    setError(null);
    try {
      setRows(await listAttendance(iso(startOfMonth()), iso(new Date())));
    } catch (err) {
      ...
      renderItem={({ item }) => (
        <View style={styles.row}>
          <View style={styles.rowLeft}>
            ...
            <Text style={styles.emp}>{t("attendance.employeeRef", { id: item.employeeId })}</Text>
          </View>
          <View style={styles.rowMid}>
            <Text style={styles.time}>{fmtTime(item.checkInAt)} → {fmtTime(item.checkOutAt)}</Text>
            <Text style={styles.worked}>{t("attendance.workedHours", { n: (item.workedMinutes / 60).toFixed(1) })}{item.lateMinutes > 0 ? ` · ${t("attendance.lateMinutes", { n: item.lateMinutes })}` : ""}</Text>
          </View>
